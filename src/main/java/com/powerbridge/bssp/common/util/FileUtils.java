...
public class FileUtils extends org.apache.commons.io.FileUtils {
    ...
    public static String uuidFileName(String fileName) {
        return UUIDGenerator.getUUID() + "." + getFileType(fileName);
    }
    ...
    public static boolean copyFile(File srcFile, String descFileName) {
        return FileUtils.copyFileCover(srcFile, descFileName, false);
    }
     ...
     * @return 如果复制成功，则返回true，否则返回false
     */
    public static boolean copyFileCover(File srcFile,
                                        String descFileName, boolean coverlay) {
//        File srcFile = new File(srcFileName);
        // 判断源文件是否存在
        if (!srcFile.exists()) {
            log.debug("复制文件失败，源文件不存在!");
            return false;
        }
        // 判断源文件是否是合法的文件
        else if (!srcFile.isFile()) {
            log.debug("复制文件失败，不是一个文件!");
            return false;
        }
        ...
        try {
            ...
            log.debug("复制单个文件 " + srcFile.getName() + " 到" + descFileName + "成功!");
            return true;
        } catch (Exception e) {
    ...
    public static boolean copyDirectoryCover(String srcDirName,
        ...
        for (int i = 0; i < files.length; i++) {
            // 如果是一个单个文件，则直接复制
            if (files[i].isFile()) {
                flag = FileUtils.copyFile(files[i],descDirName + files[i].getName());
                // 如果拷贝文件失败，则退出循环
                if (!flag) {
                    break;
                }
