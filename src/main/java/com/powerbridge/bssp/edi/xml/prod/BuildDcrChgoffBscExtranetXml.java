 ...
 * @version 1.0.0
 */
public class BuildDcrChgoffBscExtranetXml {
	static final Log logger = LogFactory.getLog(BuildDcrChgoffBscExtranetXml.class);
	...
	public void setBillData(HashMap<String,List> map, String docType,String bizType, Element dataInfoNode) {
        ...
        //创建EMS121根节点
        Element eMS121Node  = bussinessDataNode.addElement("DcrMessage");
	...
	private void buildTrInvtLt(List<DcrChgoffInvtLt> list, String docType,String bizType, Element eMS121Node) {
		String txt ="";
		String str ="";
		List<EdiXmlMapDb> ediXmlMapDbs = ediXmlInfoData.getEdiXmlInfo(docType, bizType, "DCR_CHGOFF_INVT_LT","2");
		if(list.size()>0) {
			Map<String,String> beanMap = BeanUtil.getBeanMap(list.get(0));
			//创建ET_ARCRP_ACMP_FORM_DT根节点
			for(int  i= 0; i< list.size(); i ++){
				Element node = eMS121Node.addElement("NemsDcrBillTypes");
	...
	private void buildTrImg(List<DcrChgoffImg> list, String docType,String bizType, Element eMS121Node) {
		List<EdiXmlMapDb> ediXmlMapDbs = ediXmlInfoData.getEdiXmlInfo(docType, bizType, "DCR_CHGOFF_IMG","2");
		if(list.size()>0) {
			Map<String,String> beanMap = BeanUtil.getBeanMap(list.get(0));
			//创建料件EmsChgoffDt根节点
			String txt ="";
			String str ="";
			for(int  i= 0; i< list.size(); i ++){
				Element node = eMS121Node.addElement("NmesDcrImgTypes");
				for (int j = 0; j < ediXmlMapDbs.size(); j++) {
					logger.info(ediXmlMapDbs.get(j).getXmlName());
					String xmlNmae = ediXmlMapDbs.get(j).getXmlName();
					//xmlNmae
	...
	private String buildTrHead(List<DcrChgoffBsc> list, String docType,String bizType, Element eMS121Node) {
		List<EdiXmlMapDb> ediXmlMapDbs = ediXmlInfoData.getEdiXmlInfo(docType, bizType, "DCR_CHGOFF_BSC","2");
		String chgoffTypecd = "";
		String txt ="";
		String str ="";
		if(list.size() > 0){
			Map<String,String> beanMap = BeanUtil.getBeanMap(list.get(0));
			//创建EmsChgoffBsc根节点
			Element node = eMS121Node.addElement("NemsDcrHeadType");
			for(int  i= 0; i< list.size(); i ++) {
				for (int j = 0; j < ediXmlMapDbs.size(); j++) {
					logger.info(ediXmlMapDbs.get(j).getXmlName());
					String xmlNmae = ediXmlMapDbs.get(j).getXmlName();
					...
					node.addElement(xmlNmae).addText(txt);
					if (xmlNmae.equalsIgnoreCase("chgoffTypecd")) {
						chgoffTypecd = txt;
					}
				}
			}
